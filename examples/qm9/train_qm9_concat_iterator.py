...
from chainer.dataset.convert import concat_examples, to_device
...
class ConcatSerialIterator(iterators.SerialIterator):
    ...
    def __next__(self):
        ...
        if self._order is None:
            ...
            batch = self.dataset.features[i:i_end]
        else:
            # batch = [self.dataset[index] for index in self._order[i:i_end]]
            batch = self.dataset.features[self._order[i:i_end]]
...
def converter_indentity(batch, device=None):
    return tuple([to_device(device, elem) for elem in batch])
...
def main():
    ...
    if args.label != 'all':
        labels = args.label
        cache_dir = os.path.join('input', '{}_{}_out9'.format(method, labels))
        class_num = len(labels) if isinstance(labels, list) else 1
    else:
        labels = None
        cache_dir = os.path.join('input', '{}_all_out9'.format(method))
    ...
    if dataset is None:
        print('Preprocessing dataset..., out_size=9')
        preprocessor = preprocess_method_dict[method](
            out_size=9, return_is_real_node=False)
    ...
    frac_train = args.train_data_ratio
    train, valid = splitter.train_valid_split(
        dataset, frac_train=frac_train, frac_valid=1. - frac_train)
    # frac_valid = (1 - frac_train) / 2.
    # train, valid, test = splitter.train_valid_test_split(
    #     dataset, frac_train=frac_train, frac_valid=frac_valid,
    #     frac_test=frac_valid, return_index=False)
    ...
    train_iter = ConcatSerialIterator(train, args.batchsize)
    valid_iter = ConcatSerialIterator(valid, args.batchsize, repeat=False,
                                      shuffle=False)
...
"""
Compare thease 3 scripts.
Change -g and -m option.

1. Baseline - use concat_mols and out_size differs
python train_qm9.py -g 0 -m ggnn

2. Use concat_examples, fixed out_size=9.
python train_qm9_concat_examples.py -g 0 -m ggnn

3. Use ConcatIterator, which use `features` indexer in iterator stage, 
   converter is set to identity
python train_qm9_concat_iterator.py -g 0 -m ggnn
"""