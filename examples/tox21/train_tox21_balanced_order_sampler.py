...
def main():
    ...
    if iterator_type == 'serial':
        ...
        from chainer.iterators.order_samplers.balanced_order_sampler import BalancedOrderSampler
        order_sampler = BalancedOrderSampler(
            train.features[:, -1], ignore_labels=-1, batch_balancing=True)
        order_sampler.show_label_stats()
        train_iter = SerialIterator(
            train, args.batchsize, order_sampler=order_sampler)
